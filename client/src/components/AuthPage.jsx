  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'signin') {
        ...
        const r = await signUp({ name: form.name, email: form.email, password: form.password, phone: form.phone || undefined });
        localStorage.setItem('fiq_token', r.data.token);
        localStorage.setItem('fiq_user', JSON.stringify(r.data.user));
        onAuth(r.data.user);
      }
      ...
      <div style={{ width: 440, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '3rem 2.5rem', flexShrink: 0 }}>
        <div style={{ marginBottom: '2rem' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
            {mode === 'signin' ? 'Sign in to continue tracking your gains' : 'Start your fitness journey today'}
          </p>
        </div>
